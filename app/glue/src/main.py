...
from pyspark.sql.functions import lit, date_format, current_date, col
import logging
...
class etl_desafio:
    ...
    def run(self):
        try:
            ...
            logger.info("Adicionando coluna anomesdia para particionamento")
            dataframe = dataframe.withColumn("anomesdia", lit(date_format(current_date(), "yyyyMMdd")))

            dq = DataQuality(dataframe, "utils/config_quality.json")
            dq_results = dq.run_test()

            dq_df = create_df_from_dq_results(self.spark.engine.spark_session, dq_results)
            dq_df.show()

            logger.info(f"Escrevendo tabela {database}.{tabela}")
            self.write_table(dataframe, database, tabela)
